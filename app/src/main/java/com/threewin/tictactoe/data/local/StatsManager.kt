...
class StatsManager(context: Context) {
    ...
    fun getStats(): GameStats {
        return GameStats(
            prefs.getInt(KEY_WINS, 0),
            prefs.getInt(KEY_LOSSES, 0),
            prefs.getInt(KEY_DRAWS, 0)
        )
    }

    fun incrementWins() = prefs.edit().putInt(KEY_WINS, prefs.getInt(KEY_WINS, 0) + 1).apply()
    fun incrementLosses() = prefs.edit().putInt(KEY_LOSSES, prefs.getInt(KEY_LOSSES, 0) + 1).apply()
    fun incrementDraws() = prefs.edit().putInt(KEY_DRAWS, prefs.getInt(KEY_DRAWS, 0) + 1).apply()
